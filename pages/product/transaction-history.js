...
import BalanceInformationImg from "../../public/assets/balance_information.svg";
...
export default function Products() {
  ...
  const genericHamburgerLine = `h-1 my-1 rounded-full ${
    tablet ? "bg-[#1D365F]" : "bg-[#FFFFFF]"
  } transition ease transform duration-300`;
  const genericHamburgerLine2 = `h-1 w-full my-1 rounded-full ${
    tablet ? "bg-[#1D365F]" : "bg-[#FFFFFF]"
  } transition ease transform duration-300`;
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden">
        ...
        <div
          className={`fixed w-16 sm:w-20 h-20 top-0 right-4 p-5 ${
            isOpen ? "z-[999999]" : "z-0"
          }`}
        >
          <button
            className="flex sticky top-0 right-0 w-[24px] h-[40px] flex-col justify-center group"
            style={{ visibility: isOpen ? "visible" : "hidden" }}
            onClick={() => setIsOpen(!isOpen)}
          >
            <div
              className={`${genericHamburgerLine} self-start ${
                isOpen
            ...
            />
            <div
              className={`${genericHamburgerLine} self-end ${
                isOpen
                  ? "w-full -rotate-45 -translate-y-3 opacity-100"
                  : "w-3 sm:w-4 opacity-100"
              }`}
        ...
        <Product
          ...
          text="The Transactions API allows you to access your customers’ transaction history"
          image={BalanceInformationImg}
        />
        <Footer
          onClickTop={() =>
            topSectionRef.current.scrollIntoView({ behavior: "smooth" })
